...
$sql = "SELECT username, email, phone, profile_image FROM users WHERE username = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['username']) && !empty($_POST['email']) && !empty($_POST['phone'])) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $new_username, $new_email, $new_phone, $username_session);
        if ($stmt->execute()) {
            // Update session dan variabel lokal
    ...
    <div class="container-xxl bg-white p-0">
        <div class="d-flex">
            ...
            <div class="d-flex flex-column flex-shrink-0 p-3"
                style="width: 220px; height: 100vh; background-color: #00765a;">
                ...
                <ul class="nav nav-pills flex-column mb-auto">
                    ...
                    <li>
                        <a href="pk-dashboard-kost.php" class="nav-link text-light">
                            <i class="bi bi-house-door me-2"></i>Kost
                        </a>
                    </li>
                    <li>
                ...
                <div class="dropdown">
                    <a href="#" class="d-flex align-items-center text-light text-decoration-none dropdown-toggle"
                        ...
                        <img src="<?php echo htmlspecialchars($profile_image); ?>" alt="Admin" width="32" height="32"
                    ...
                    <div class="col-lg-4 mb-4 mt-3">
                        <div class="card text-center">
                            <img src="<?php echo htmlspecialchars($profile_image); ?>" alt="Profile Picture"
                                class="card-img-top rounded-circle mx-auto mt-3" style="width: 150px;">
                            <div class="card-body">
                    ...
                    <div class="col-md-8 mt-3">
                        ...
                        <form method="POST" action="" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="tel" class="form-control" id="ownerPhone" name="phone"
                                    value="<?php echo htmlspecialchars($phone); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="profileImage" class="form-label">Ubah Foto Profil</label>
...
// Ambil data pengguna dari database
$sql = "SELECT username, email, nomor_hp, profile_image FROM logsys_pk WHERE username = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $username_session); // Gunakan session username
$stmt->execute();
$result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $uploadFile)) {
                // Simpan path file gambar ke database
                $sql = "UPDATE logsys_pk SET profile_image = ? WHERE username = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ss", $uploadFile, $username_session);
                    ...
                    <div class="col-lg-4 mb-4 mt-3">
                        <div class="profile-card text-center">
                            ...
                            <?php if (isset($profile_image) && file_exists($profile_image)): ?>
                                <img id="profile_image_preview" src="<?php echo $profile_image; ?>" alt="Profile_Image"
